...
NeomorphFrameLayout layout_logo, layout_logo2;
            ...
            if(Double.parseDouble(keyarray[2])>0.9){
                ...
                for(int i=0;i<SQLdatabase.kitaplarliste.size();i++){
                    if(SQLdatabase.kitaplarliste.get(i).getYazarid()==yazarid){
                        int sesid=SQLdatabase.kitaplarliste.get(i).getSesid();
                        for(int j=0;j<SQLdatabase.seslerlister.size();j++){
                            if(sesid==SQLdatabase.seslerlister.get(j).getSesid()){
                                t1.SırayaekleveOku(SQLdatabase.seslerlister.get(j).getSüresi());//süresi okunuşu demek
                            }
                        }
    ...
    public void Kitapokumaactivty(String ismi){
        ...
        Intent adminintent = new Intent(getApplicationContext(),new Kitapplayer().getClass());
